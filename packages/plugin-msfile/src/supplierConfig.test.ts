...
describe("normalizeSupplierAddress", () => {
  it("accepts webrtc-direct with matching peer and certhash", () => {
    const address = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uEiDu8SJ7IdK9W_PfRJfV0clhOP6mG0zNXcZQ8bBhC9ipwg/p2p/${SUPPLIER_PEER_ID}`;
  ...
  it("requires /certhash on webrtc-direct", () => {
    const address = `/dns4/nas.example.com/udp/4001/webrtc-direct/p2p/${SUPPLIER_PEER_ID}`;
    const result = normalizeSupplierAddress(address, SUPPLIER_PEER_ID, DEFAULT_ADDRESS_POLICY);
    expect(!result.ok && result.message).toMatch(/certhash/);
  });
  ...
  it("rejects mismatched /p2p peer ids", () => {
    const address = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uEiDu8SJ7IdK9W_PfRJfV0clhOP6mG0zNXcZQ8bBhC9ipwg/p2p/${OWNER_PUBKEY}`;
    const result = normalizeSupplierAddress(address, SUPPLIER_PEER_ID, DEFAULT_ADDRESS_POLICY);
  ...
  it("rejects malformed transport sequences（审查修复）", () => {
    ...
    const hybrid = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uEiDu8SJ7IdK9W_PfRJfV0clhOP6mG0zNXcZQ8bBhC9ipwg/tcp/443/tls/ws/p2p/${SUPPLIER_PEER_ID}`;
    ...
    const shortCert = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uEiBAD/p2p/${SUPPLIER_PEER_ID}`;
    ...
    if (!shortResult.ok) expect(shortResult.message).toMatch(/sha256|32 bytes|base64url/);
    // 非 sha256 multihash code（0x00 identity）。
    const wrongCode = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uAAiECJcqEQZmPj5Z3Sko8HcOyBhtttmX_cRcpw5cL6zVVEA/p2p/${SUPPLIER_PEER_ID}`;
    ...
    if (!wrongCodeResult.ok) expect(wrongCodeResult.message).toMatch(/sha256/);
    // 组件顺序颠倒：certhash 在 webrtc-direct 之前。
    const reordered = `/dns4/nas.example.com/certhash/uEiDu8SJ7IdK9W_PfRJfV0clhOP6mG0zNXcZQ8bBhC9ipwg/udp/4001/webrtc-direct/p2p/${SUPPLIER_PEER_ID}`;
...
describe("normalizeSupplierDraft", () => {
  const validAddress = `/dns4/nas.example.com/udp/4001/webrtc-direct/certhash/uEiDu8SJ7IdK9W_PfRJfV0clhOP6mG0zNXcZQ8bBhC9ipwg/p2p/${SUPPLIER_PEER_ID}`;
