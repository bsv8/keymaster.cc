...
async function persist(
  ...
  if (prior) {
    if (!equal(prior, bytes)) throw new Error("BitFS 买方签名证据与已保存字节冲突");
    if (!record.evidence.includes(name)) await input.sessions.putEvidence(input.sessionId, record.revision, name, bytes, input.nowMs());
    return;
  }
