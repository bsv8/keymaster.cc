...
function rpc() {
  ...
  return {
    calls,
    msfileExecutorSignNoiseStaticKey: async (input: Record<string, unknown>) => {
      calls.push({ kind: "noise", input });
      return { status: "ok", value: { signatureDer: DER.slice(0) } };
    },
    msfileExecutorSignPeerRecord: async (input: Record<string, unknown>) => {
...
describe("KeymasterMsFileIdentitySigner", () => {
  it("implements only the two typed signing methods and keeps identity public", async () => {
    const bridge = rpc();
    const signer = new KeymasterMsFileIdentitySigner({ leaseId: "lease-1", sessionEpoch: "epoch-1", activePublicKeyHex: PUBLIC_KEY_HEX, rpc: bridge });
  ...
  it("rejects short Noise keys, non-empty addresses, and a PeerId for another key", async () => {
    const signer = new KeymasterMsFileIdentitySigner({ leaseId: "lease-1", sessionEpoch: "epoch-1", activePublicKeyHex: PUBLIC_KEY_HEX, rpc: rpc() });
  ...
  it("propagates AbortSignal to the dedicated RPC", async () => {
    ...
    const bridge = new KeymasterMsFileIdentitySigner({
      leaseId: "lease-1",
      sessionEpoch: "epoch-1",
      activePublicKeyHex: PUBLIC_KEY_HEX,
      rpc: {
        msfileExecutorSignNoiseStaticKey: (_input, signal) => new Promise((_, reject) => {
          signal?.addEventListener("abort", () => { sawAbort = true; reject(new Error("aborted")); }, { once: true });
        }),
        msfileExecutorSignPeerRecord: async () => ({ status: "ok", value: { signatureDer: DER.slice(0) } })
      }
    });
