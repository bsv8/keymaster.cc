...
describe("BSV-21 / STAS / 1Sat WOC 路径映射", () => {
  it("getRawTransaction 使用当前 WOC raw transaction 路径并接受裸 hex 文本", async () => {
    installFetchMock((url) => {
      fetchLog.push(url);
      return new Response("01000000", { status: 200, headers: { "content-type": "text/plain" } });
    });
    const bus = createMessageBus();
    const svc = createWocService({ messageBus: bus });
    await expect(svc.getRawTransaction!(MAIN, "ab".repeat(32))).resolves.toBe("01000000");
    expect(fetchLog[0]).toMatch(/\/v1\/bsv\/main\/tx\/(?:ab){32}\/hex$/);
    svc.dispose();
  });

  it("getRawTransaction 保持兼容 JSON object 响应", async () => {
    installFetchMock(() => new Response(JSON.stringify({ hex: "00" }), { status: 200 }));
    const bus = createMessageBus();
    const svc = createWocService({ messageBus: bus });
