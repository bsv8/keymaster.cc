...
function makeDb() {
  ...
  return { resource, claims, locals, outputs, async getResource(id: string) { return id === resource.resourceId ? resource : undefined; }, async listUtxos() { return [utxo]; }, async listLocalInputClaimsByResource() { return [...claims.values()]; }, async listLocalTransactions() { return [...locals.values()]; }, async listLocalOutpoints() { return [...outputs.values()]; }, async prepareLocalSubmission(input: { submission: P2pkhLocalTransaction; claims: P2pkhLocalInputClaim[]; localOutpoints: P2pkhLocalOutpoint[] }) { locals.set(input.submission.id, input.submission); for (const claim of input.claims) claims.set(claim.id, claim); for (const output of input.localOutpoints) outputs.set(output.id, output); }, async finishLocalSubmission(input: { submissionId: string; state: "local-confirmed" | "isolated" | "chain-confirmed" | "conflicted" }) { const row = locals.get(input.submissionId)!; locals.set(row.id, { ...row, state: input.state }); for (const claim of claims.values()) if (claim.submissionId === input.submissionId) claims.set(claim.id, { ...claim, state: input.state === "local-confirmed" ? "active" : "isolated" }); for (const output of outputs.values()) if (output.submissionId === input.submissionId) outputs.set(output.id, { ...output, state: input.state === "local-confirmed" ? "available" : "isolated" }); }, async abortUnattemptedLocalSubmission() {} };
}

function createFixture(outcome: "accepted" | "isolated") {
  const db = makeDb(); const emit = vi.fn();
  const service = createP2pkhTransferService({ vault: makeVault(), messageBus: { publish: vi.fn(), subscribe: vi.fn(() => () => undefined) } as never, assetDataNotifier: { emit, subscribe: vi.fn() } as unknown as AssetDataNotifier, getDb: async () => db as never, broadcastPreflight: async () => ({ generation: 1 }), broadcastWithCoordinator: async ({ submissionId }: { submissionId: string }) => { await db.finishLocalSubmission({ submissionId, state: outcome === "isolated" ? "isolated" : "local-confirmed" }); return { status: "ok" as const, value: { status: outcome }, sessionEpoch: "test-epoch" }; }, getActiveKey: () => ({ publicKeyHex: OWNER.publicKeyHex, label: "active", capabilities: [], createdAt: "now" }), getKeyForOwner: async (publicKeyHex) => ({ publicKeyHex, label: "active", capabilities: [], createdAt: "now" }) });
  return { service, db, emit };
}
...
describe("p2pkh transfer data notifications", () => {
  ...
  it("emits after isolation and keeps the local failure visible", async () => {
    const { service, db, emit } = createFixture("isolated");
    const preview = await service.prepare({ ownerPublicKeyHex: OWNER.publicKeyHex, assetId: "bsv", recipientAddress: RECIPIENT.address, amountSatoshis: 1_000, feeRateSatoshisPerKb: 1 });
