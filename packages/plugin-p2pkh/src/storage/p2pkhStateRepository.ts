...
type RecordStore = P2pkhStateCollection;
type RecordIndex = P2pkhStateIndex;
type RecordTransaction = P2pkhStateTransaction;

function request<T>(value: Promise<T>): Promise<T> { return value; }

async function runTransaction<T>(handle: P2pkhStateRepositoryBundle, stores: string | string[], mode: "readonly" | "readwrite", body: (tx: RecordTransaction) => Promise<T>): Promise<T> {
  const recordStore = handle.getStore();
  const snapshot = recordStore.snapshot();
  const tx = recordStore.transaction(stores, mode);
  try {
    const result = await body(tx);
    if (mode === "readwrite") await recordStore.flush();
    return result;
  } catch (error) {
    recordStore.restore(snapshot);
    throw error;
  }
}

function all<T>(store: RecordStore | RecordIndex): Promise<T[]> { return store.getAll() as Promise<T[]>; }
function allByKey<T>(store: RecordStore | RecordIndex, key: RecordQuery): Promise<T[]> { return store.getAll(key) as Promise<T[]>; }
function getById<T>(store: RecordStore, id: RecordKey): Promise<T | undefined> { return store.get(id) as Promise<T | undefined>; }
async function pageByCursor<T>(store: RecordStore | RecordIndex, query: RecordQuery, direction: "next" | "prev", limit: number): Promise<T[]> {
  const rows = await store.getAll(query) as T[];
  return direction === "prev" ? rows.reverse().slice(0, limit) : rows.slice(0, limit);
}
function encodePageCursor(key: RecordKey): string { return encodeURIComponent(JSON.stringify(key)); }
function decodePageCursor(cursor: string | undefined): RecordKey | undefined {
  if (!cursor) return undefined;
  try {
    const value = JSON.parse(decodeURIComponent(cursor)) as unknown;
    return Array.isArray(value) || typeof value === "string" || typeof value === "number" ? value as RecordKey : undefined;
  } catch { return undefined; }
}
async function pageByIndex<T>(index: RecordIndex, resourceId: string, cursor: string | undefined, limit: number): Promise<{ items: T[]; nextCursor?: string }> {
  const rows = await index.getAll(recordKeyRange([resourceId, "", ""], [resourceId, "\uffff", "\uffff"])) as T[];
  const after = decodePageCursor(cursor);
  const filtered = after === undefined ? rows : rows.filter((row) => comparePageKey(row, after) < 0);
  const items = filtered.reverse().slice(0, limit);
  const next = filtered.length > items.length ? encodePageCursor(extractPageKey(items[items.length - 1])) : undefined;
  return next ? { items, nextCursor: next } : { items };
}
function extractPageKey(value: unknown): RecordKey { const row = value as Record<string, unknown>; return [String(row.resourceId ?? ""), String(row.updatedAt ?? row.lastConfirmedAt ?? ""), String(row.id ?? row.txid ?? "")]; }
function comparePageKey(value: unknown, target: RecordKey): number { return JSON.stringify(extractPageKey(value)).localeCompare(JSON.stringify(target)); }
...
export async function openP2pkhStateRepository(store: KeyValueStore): Promise<P2pkhStateRepositoryBundle> {
  ...
  const bundle: P2pkhStateRepositoryBundle = {
    ...
    close: () => { if (closed) return; closed = true; openHandles.delete(bundle); recordStore.close(); }
  };
 ...
 */
async function normalizeLocalOutputGroupsInTransaction(tx: RecordTransaction, resourceId: string, now: string, affectedOutpointKeys: Iterable<string>): Promise<void> {
...
export function createP2pkhStateRepository(handle: P2pkhStateRepositoryBundle) {
  return {
    getStore: () => handle.getStore(), close: () => handle.close(),
    replaceLocalTransaction: (row: P2pkhLocalTransaction) => runTransaction(handle, "p2pkh_local_transactions", "readwrite", async (tx) => { await request(tx.objectStore("p2pkh_local_transactions").put(row)); }),
    ...
    async ingestConfirmedTransactionPage(input: { resource: P2pkhKeyResource; transactions: Array<{ txid: string; rawTxHex: string; blockHeight?: number; blockHash?: string; blockTime?: number }>; syncState: P2pkhTransactionSyncState; reorgCheck?: { observedTxids: string[]; completeHistory: boolean; anchorTxid?: string } }): Promise<void> {
      ...
      await runTransaction(handle, ["p2pkh_addresses", "p2pkh_transactions", "p2pkh_owned_outpoints", "p2pkh_transaction_sync", "p2pkh_local_transactions", "p2pkh_local_input_claims", "p2pkh_local_outpoints"], "readwrite", async (tx) => {
        ...
        if (reorgCheck && (reorgCheck.completeHistory || reorgCheck.anchorTxid)) {
          ...
          const existingFacts = reorgCheck.completeHistory
            ...
            : anchor?.blockHeight === undefined
              ? []
              : await allByKey<P2pkhTransactionFact>(facts.index("resourceBlockHeight"), recordKeyRange([input.resource.resourceId, anchor.blockHeight], [input.resource.resourceId, Number.MAX_SAFE_INTEGER]));
          ...
          if (reorgCheck.completeHistory) {
            completeOwnedValueByOutpoint = new Map((await allByKey<P2pkhOwnedOutpointProjection>(ownedStore.index("resourceOutpointKey"), recordKeyRange([input.resource.resourceId, ""], [input.resource.resourceId, "\uffff"]))).map((row) => [row.outpointKey, row.value]));
        ...
        if (reorgCheck?.completeHistory) {
          ...
          // repeats the same descendant walk (and platform K-V repository writes) for every
    ...
    clearAll: async () => { const names = [...handle.getStore().objectStoreNames].filter((name) => name.startsWith(STORE_PREFIX)); await runTransaction(handle, names, "readwrite", async (tx) => { for (const name of names) await request(tx.objectStore(name).clear()); }); }
  };
}
