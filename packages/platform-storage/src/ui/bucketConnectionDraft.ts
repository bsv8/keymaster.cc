...
import { createBrowserLocalBucketProvider } from "../browser/browserBucketProvider.js";
 ...
 *
 * 页面存储句柄来自设备引导模块的统一浏览器入口；UI 侧不得直接解析
 * 浏览器存储对象，存储门禁用它（见 check-storage-hard-switch）。
 *
...
export function createBucketProvider(input: BucketProviderInput, bucketId: string): StorageBucketProvider {
  if (isBucketDraft(input)) {
    if (input.backend === "local") return createBrowserLocalBucketProvider(bucketId);
    return createS3BucketProvider(normalizedProviderConfigFromBucketDraft(input), { bucketId });
  }
  if (input.kind === "local") return createBrowserLocalBucketProvider(bucketId);
