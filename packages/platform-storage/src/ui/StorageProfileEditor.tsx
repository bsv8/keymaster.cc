...
export function StorageProfileEditor() {
  ...
  const [busy, setBusy] = useState<"probe" | "capability" | "activate" | "unlock" | "clear" | null>(null);
  ...
  function updateProfilePassword(value: string) { setProfilePassword(value); markEdited(); }
  function draft(): StorageProviderConfigDraft {
    return {
      providerId,
      connection,
      credentials: replaceCredentials || !summary ? { mode: "replace", accessKeyId, secretAccessKey } : { mode: "retain" },
      ...(profilePassword.length > 0 ? { profilePassword } : {})
    };
  }
  function isDraftComplete(config = draft()): boolean {
    return isConnectionComplete(config.providerId, config.connection)
      ...
      && (Boolean(summary) && resource.status === "ready" || Boolean(config.profilePassword && config.profilePassword.length >= 8));
  }
    ...
    try {
      while (pendingActivationRef.current) {
        ...
        try {
          ...
          if (("status" in result && result.status !== "selected") || ("ok" in result && !result.ok)) {
            throw new Error("diagnostic" in result ? result.diagnostic ?? "activation failed" : "activation failed");
          }
  ...
  async function unlockProfile() {
    if (profilePassword.length < 8) {
      setMessage(t("storage.settings.profilePasswordRequired"));
      setMessageTone("error");
      return;
    }
    setBusy("unlock"); setMessage(null);
    try {
      const result = await service.unlockStorageProfile(profilePassword);
      if (!result.ok) throw new Error(result.diagnostic ?? "authentication");
      setProfilePassword("");
      setMessage(t("storage.settings.profileUnlocked"));
      setMessageTone("success");
    } catch (error) { showError(error); }
    finally { setBusy(null); }
  }
  async function selectOpfs() {
    setBackendBusy(true); setMessage(null);
    try {
      const result = await service.selectOpfs();
      if (!result.ok) throw new Error(result.diagnostic ?? "OPFS activation failed");
      setMessage("OPFS 已选择并验证"); setMessageTone("success");
  ...
  async function importProfile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (profilePassword.length < 8) {
      setMessage(t("storage.settings.profilePasswordRequired")); setMessageTone("error"); return;
    }
    setBackendBusy(true); setMessage(null);
    try {
      const envelope = importStorageProfileEnvelope(new Uint8Array(await file.arrayBuffer()));
      const result = await service.importStorageProfile(envelope, profilePassword);
      if (!result.ok) throw new Error(result.diagnostic ?? "Profile import failed");
      setProfilePassword(""); setMessage("Storage Profile 已导入并激活"); setMessageTone("success");
      window.location.reload();
    } catch (error) { showError(error); }
    finally { setBackendBusy(false); }
  }
  ...
  const editingDisabled = busy !== null && busy !== "activate" && busy !== "unlock";
  ...
  const profilePasswordRequired = !summary || resource.status !== "ready";
  ...
  const capabilityRow = (label: string, value: BucketConditionalCapabilityView | undefined, testId: string) => (
  ...
  return (
    <div className="storage-settings">
      ...
      <section className="storage-settings__section">
        ...
        <div className="storage-settings__fields storage-settings__fields--credentials">
          <label className="storage-settings__field storage-settings__field--full">{t("storage.settings.profilePassword")}
            <input type="password" autoComplete="current-password" value={profilePassword} onChange={(event) => updateProfilePassword(event.target.value)} disabled={editingDisabled} required={profilePasswordRequired} />
          </label>
          {resource.healthStatus === "authentication" || (summary && resource.status !== "ready") ? <Button type="button" variant="secondary" onClick={() => void unlockProfile()} disabled={busy !== null || profilePassword.length < 8}>{t("storage.settings.unlockProfile")}</Button> : null}
        </div>
      ...
      <div className="storage-settings__actions">
        <div>
          <Button type="button" variant="secondary" onClick={() => void selectOpfs()} disabled={backendBusy || busy !== null}>{t("storage.settings.selectOpfs")}</Button>
          <label className="storage-settings__import-profile">
            <span className="storage-settings__button">{t("storage.settings.importProfile")}</span>
            <input type="file" accept="application/json,.json" onChange={(event) => void importProfile(event)} disabled={backendBusy || busy !== null} />
          </label>
