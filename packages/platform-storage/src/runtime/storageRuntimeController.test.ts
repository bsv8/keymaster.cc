...
import type { NormalizedStorageProviderConfig, OwnerAppStorageGrant, StorageSecretEnvelope } from "@keymaster/contracts";
import type { BucketObjectStore, BucketObjectStoreCapabilityState } from "../bucket-providers/bucketObjectStore.js";
import { createStorageRuntimeController } from "./storageRuntimeController.js";
import { setBucketObjectStoreCapabilityMode } from "../bucket-providers/bucketObjectStore.js";
...
import { StorageRuntimeError } from "./storageRuntimeError.js";
...
function makeRepository(): MultipartUploadRepository & { uploads: Map<string, StoredMultipartUploadRecord> } {
...
function makeSecret() {
  return {
    seal: async (_scope: string, plaintext: Uint8Array): Promise<StorageSecretEnvelope> => ({ version: 2, saltHex: "", nonceHex: "", ciphertextHex: bytesToHex(plaintext) }),
    open: async (_scope: string, sealed: StorageSecretEnvelope) => hexToBytes(sealed.ciphertextHex)
  };
}

function makeStore(calls: { listPrefixes: string[]; abortedUploads?: string[] }): BucketObjectStore {
  ...
  return {
    ...
    async put(input) {
      if (input.ifNoneMatch === "*" && objects.has(input.key)) throw new StorageRuntimeError("storage_conflict");
    ...
    async get(input) {
      ...
      if (!value) throw new StorageRuntimeError("storage_not_found");
    ...
    async uploadPart(input) {
      ...
      if (!upload) throw new StorageRuntimeError("storage_not_found");
    ...
    async completeMultipart(input) {
      const upload = multipart.get(input.uploadId);
      if (!upload) throw new StorageRuntimeError("storage_not_found");
      if (input.ifNoneMatch === "*" && objects.has(upload.key)) throw new StorageRuntimeError("storage_conflict");
...
function makeCapabilityStore(state: BucketObjectStoreCapabilityState, behavior: { put: CapabilityBehavior; complete: CapabilityBehavior }, options: { cleanupFailure?: boolean; calls?: CapabilityCall[]; gate?: Promise<void> } = {}): BucketObjectStore {
  ...
  return {
    ...
    async put(input) {
      ...
      if (input.ifNoneMatch) {
        ...
        if (attempt === 1) {
          ...
          if (behavior.put === "inconclusive") throw new StorageRuntimeError("storage_provider_error");
          setBucketObjectStoreCapabilityMode(state, "put", behavior.put === "best-effort" ? "best-effort" : "native", "automatic");
          return base.put(input);
        }
        if (behavior.put === "silent") return base.put({ ...input, ifNoneMatch: undefined });
        if (behavior.put === "second-error") throw new StorageRuntimeError("storage_provider_error");
        throw new StorageRuntimeError("storage_conflict");
      }
      return base.put(input);
    },
    async completeMultipart(input) {
      ...
      if (input.ifNoneMatch) {
        ...
        if (attempt === 1) {
          if (behavior.complete === "inconclusive") throw new StorageRuntimeError("storage_provider_error");
          setBucketObjectStoreCapabilityMode(state, "complete", behavior.complete === "best-effort" ? "best-effort" : "native", "automatic");
          return base.completeMultipart(input);
        }
        if (behavior.complete === "silent") return base.completeMultipart({ ...input, ifNoneMatch: undefined });
        if (behavior.complete === "second-error") throw new StorageRuntimeError("storage_provider_error");
        throw new StorageRuntimeError("storage_conflict");
      }
      return base.completeMultipart(input);
    },
    ...
    async delete(input) { options.calls?.push({ operation: "delete", namespaceRoot: input.namespaceRoot, key: input.key }); if (options.cleanupFailure) throw new StorageRuntimeError("storage_provider_error"); return base.delete(input); },
    async abortMultipart(input) { options.calls?.push({ operation: "abort", namespaceRoot: input.namespaceRoot, key: input.key, uploadId: input.uploadId }); if (options.cleanupFailure) throw new StorageRuntimeError("storage_provider_error"); return base.abortMultipart(input); }
  };
}
...
const sessionEpoch = "epoch-a";

function context(appIdentity = identityA): OwnerAppStorageGrant {
  return { connectSessionId: "session-a", transportOrigin: "https://app.example", appIdentity, bucketId, bucketGeneration, ownerPublicKeyHex, applicationStorageId: deriveThirdPartyApplicationStorageId(appIdentity.publisherPublicKeyHex, appIdentity.appId), sessionEpoch };
}

async function createAbortService(mode: "success" | "reject" | "timeout" | "nostore" | "old-generation") {
  const multipartUploadRepository = makeRepository();
  const calls = { listPrefixes: [] as string[], abortedUploads: [] as string[] };
  const base = makeStore(calls);
  const store: BucketObjectStore = mode === "timeout"
    ? { ...base, async abortMultipart() { await new Promise<void>(() => undefined); } }
    : mode === "reject"
      ? { ...base, async abortMultipart() { throw new StorageRuntimeError("storage_provider_error"); } }
      : base;
  const service = await createStorageRuntimeController({
    multipartUploadRepository, secret: makeSecret(),
  ...
  const applicationStorageId = deriveThirdPartyApplicationStorageId(identityA.publisherPublicKeyHex, identityA.appId);
  await multipartUploadRepository.putMultipart({ internalUploadId: `internal-${mode}`, connectSessionId: "session-a", transportOrigin: "https://app.example", ownerPublicKeyHex, applicationStorageId, bucketId, bucketGeneration, sessionEpoch, relativePath: "file.bin", physicalKey: `${ownerPublicKeyHex}/${applicationStorageId}/file.bin`, sealedS3UploadId: { version: 2, saltHex: "", nonceHex: "", ciphertextHex: bytesToHex(new TextEncoder().encode("remote-upload")) }, providerGeneration: mode === "old-generation" ? 999 : 1, expectedSize: 0, overwrite: false, parts: [], expiresAt: Date.now() + 60_000, createdAt: Date.now() });
  if (mode === "nostore") service.dispose();
  return { service, multipartUploadRepository };
}
...
async function createCapabilityService(behavior: { put: CapabilityBehavior; complete: CapabilityBehavior }, options: { cleanupFailure?: boolean; calls?: CapabilityCall[]; gate?: Promise<void> } = {}) {
  const states: BucketObjectStoreCapabilityState[] = [];
  const service = await createStorageRuntimeController({
    multipartUploadRepository: makeRepository(), secret: makeSecret(),
...
describe("StorageRuntimeControllerImpl", () => {
  it("cancels a never-resolving probe so the mutation lane recovers immediately", async () => {
    ...
    const service = await createStorageRuntimeController({ multipartUploadRepository: makeRepository(), secret: makeSecret(), objectStoreFactory: () => ({ ...base, async probe() { if (hanging) await new Promise<void>(() => undefined); } }) });
  ...
  it("completes clear while a provider data request never settles", async () => {
    const base = makeStore({ listPrefixes: [] });
    const service = await createStorageRuntimeController({ multipartUploadRepository: makeRepository(), secret: makeSecret(), objectStoreFactory: () => ({ ...base, async list() { return await new Promise<never>(() => undefined); } }) });
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
  ...
  it("closes the clear gate before snapshot and cleans the in-flight multipart", async () => {
    const multipartUploadRepository = makeRepository(); const calls = { listPrefixes: [] as string[], abortedUploads: [] as string[] }; const base = makeStore(calls); let release!: () => void;
    const store: BucketObjectStore = { ...base, async uploadPart() { await new Promise<void>((resolve) => { release = resolve; }); return "late-etag"; } };
    const service = await createStorageRuntimeController({ multipartUploadRepository, secret: makeSecret(), objectStoreFactory: () => store });
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
    ...
    expect(await multipartUploadRepository.getMultipart(upload.uploadId)).toBeNull(); service.dispose();
  });

  it("serializes concurrent parts so neither durable part update is lost", async () => {
    const multipartUploadRepository = makeRepository();
    const service = await createStorageRuntimeController({ multipartUploadRepository, secret: makeSecret(), objectStoreFactory: () => makeStore({ listPrefixes: [] }) });
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
    ...
    const record = await multipartUploadRepository.getMultipart(begin.uploadId);
  ...
  it("deletes durable multipart only after remote abort succeeds", async () => {
    const { service, multipartUploadRepository } = await createAbortService("success");
    await service.abortSession("session-a");
    expect(await multipartUploadRepository.getMultipart("internal-success")).toBeNull();
  });

  it.each(["reject", "timeout", "nostore", "old-generation"] as const)("retains durable multipart when cleanup is %s", async (mode) => {
    const { service, multipartUploadRepository } = await createAbortService(mode);
    await service.abortSession("session-a");
    expect(await multipartUploadRepository.getMultipart(`internal-${mode}`)).not.toBeNull();
  }, 3000);

  it("bounds session cleanup globally across many hanging uploads", async () => {
    const { service, multipartUploadRepository } = await createAbortService("timeout");
    const seed = await multipartUploadRepository.getMultipart("internal-timeout");
    for (let i = 0; i < 7; i++) await multipartUploadRepository.putMultipart({ ...seed!, internalUploadId: `internal-timeout-${i}` });
    const started = Date.now(); await service.abortSession("session-a");
    expect(Date.now() - started).toBeLessThan(1500);
    expect((await multipartUploadRepository.listMultiparts()).length).toBe(8);
  }, 2500);
  ...
  it("pins list prefixes to the app namespace and rejects a foreign cursor", async () => {
    ...
    expect(calls.listPrefixes[0]).toBe(`${ownerPublicKeyHex}/${deriveThirdPartyApplicationStorageId(identityA.publisherPublicKeyHex, identityA.appId)}/`);
  ...
  it("shares capability state across lock/unlock and replaces it on provider generation changes", async () => {
    ...
    setBucketObjectStoreCapabilityMode(firstGenerationState!, "put", "native", "automatic");
    ...
    expect(states[1]).not.toBe(firstGenerationState);
    service.dispose();
  });
  ...
  it("rejects unsafe range arithmetic and provider range metadata", async () => {
    ...
    const service = await createStorageRuntimeController({ multipartUploadRepository: makeRepository(), secret: makeSecret(), objectStoreFactory: () => ({ ...base, async get() { return response; } }) });
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
  ...
  it("maps oversized path segments to storage_invalid_path", async () => {
    const service = await createStorageRuntimeController({ multipartUploadRepository: makeRepository(), secret: makeSecret(), objectStoreFactory: () => makeStore({ listPrefixes: [] }) });
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
  ...
  it("best-effort aborts uploads and atomically retires local state on Clear", async () => {
    ...
    await expect(multipartUploadRepository.getProviderConfig()).resolves.toBeNull();
    await expect(multipartUploadRepository.listMultiparts()).resolves.toEqual([]);
  ...
  it("keeps the old client usable and local upload records intact when Clear commit fails", async () => {
    ...
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket-name" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
    await service.beginUpload(context(), { path: "pending.bin", size: 1 });
    multipartUploadRepository.clearProviderConfig = async () => { throw new Error("fixture clear failure"); };
    ...
    await expect(multipartUploadRepository.getProviderConfig()).resolves.not.toBeNull();
    await expect(multipartUploadRepository.listMultiparts()).resolves.toHaveLength(1);
  ...
  it("re-arms request cancellation and preserves the old client when reset fails", async () => {
    ...
    await service.activateProvider({ providerId: "aws-s3", connection: { region: "us-east-1", bucket: "bucket-name" }, credentials: { mode: "replace", accessKeyId: "key", secretAccessKey: "secret" } });
    await service.beginUpload(context(), { path: "pending.bin", size: 1 });
    multipartUploadRepository.resetStorage = async () => { throw new Error("fixture reset failure"); };
