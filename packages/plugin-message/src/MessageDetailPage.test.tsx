...
function makeFakeHost(service: MessageService | null, webrtcService?: WebrtcService | null): PluginHost {
  ...
  resourceRegistry.register({
    id: "message.detail",
    scope: "active-key",
    key: (args: readonly string[], context: { activePublicKeyHex?: string }) =>
      ["message.detail", context.activePublicKeyHex ?? "none", args[0] ?? "none"],
    load: async (args: readonly string[]) => {
  ...
  const resourceStore = {
    ...
    subscribe: (definitionId: string, args: readonly string[], callback: () => void) => {
      ...
      const invalidate = () => {
        void def.load(args, context, new AbortController().signal).then((data: any) => {
          record.snapshot = { key, status: "ready", data, revision: record.snapshot.revision + 1 };
          for (const sub of record.subscribers) sub();
        });
      };
      const offDefinition = def.subscribe?.(args, context, invalidate) ?? (() => {});
      return () => { offDefinition(); record.subscribers.delete(callback); };
    },
