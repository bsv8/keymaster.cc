...
const resources: I18nPluginResources = {
  ...
  resources: {
    en: {
      ...
      "sat.settings.billing.description": "Billing is queried directly from the SS server and is not written to setting.json. Current pages are in the supplier panels above; this is the latest-20 summary.",
      "sat.settings.billing.refresh": "Query server billing",
      "sat.settings.billing.refreshed": "Server billing refreshed.",
      "sat.settings.billing.panel": "Server billing (queried live from SS server, not saved locally)",
      "sat.settings.billing.pageSize": "Rows per page (billing pagination)",
      "sat.settings.billing.pageStatus": "Billing page status",
      "sat.settings.billing.notQueried": "Server billing has not been queried yet",
      "sat.settings.billing.prev": "Previous page",
      "sat.settings.billing.next": "Next page",
      "sat.settings.billing.hasMore": "has next page",
      "sat.settings.billing.noMore": "last page",
      "sat.settings.connectionState.online": "Connected (green)",
      "sat.settings.connectionState.degraded": "Degraded (yellow)",
      "sat.settings.connectionState.connecting": "Connecting (yellow)",
      "sat.settings.connectionState.disabled": "Disabled (gray)",
      "sat.settings.connectionState.disconnected": "Disconnected (red)",
      "sat.settings.spi.refresh": "Refresh SPI balance",
      "sat.settings.spi.refreshed": "SPI balance refreshed.",
      "sat.settings.spi.topupAmount": "Top-up amount (satoshis, positive integer)",
      "sat.settings.spi.collectAmount": "Collect amount (satoshis, positive integer)",
    ...
    "zh-CN": {
      ...
      "sat.settings.billing.description": "账单直接查询 SS server，不写入 setting.json。各供应商当前页见上方账单面板，这里是汇总（最新 20 条）。",
      "sat.settings.billing.refresh": "查询服务器账单",
      "sat.settings.billing.refreshed": "服务器账单已刷新。",
      "sat.settings.billing.panel": "服务器账单（SS server 直查，不写入本地）",
      "sat.settings.billing.pageSize": "每页条数（账单分页）",
      "sat.settings.billing.pageStatus": "账单分页状态",
      "sat.settings.billing.notQueried": "尚未查询服务器账单",
      "sat.settings.billing.prev": "上一页",
      "sat.settings.billing.next": "下一页",
      "sat.settings.billing.hasMore": "还有下一页",
      "sat.settings.billing.noMore": "已是最后一页",
      "sat.settings.connectionState.online": "已连接（绿灯）",
      "sat.settings.connectionState.degraded": "已降级（黄灯）",
      "sat.settings.connectionState.connecting": "连接中（黄灯）",
      "sat.settings.connectionState.disabled": "已停用（灰灯）",
      "sat.settings.connectionState.disconnected": "未连接（红灯）",
      "sat.settings.spi.refresh": "刷新 SPI 余额",
      "sat.settings.spi.refreshed": "SPI 余额已刷新。",
      "sat.settings.spi.topupAmount": "充值金额（satoshis，正整数）",
      "sat.settings.spi.collectAmount": "回收金额（satoshis，正整数）",
