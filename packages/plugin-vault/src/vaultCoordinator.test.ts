...
import { buildVaultAuthMetadata, resolveVaultPasswordKey, verifyVaultPasswordKey } from "./vaultCoordinator.js";

describe("vaultCoordinator auth metadata", () => {
  it("builds canonical v2 auth metadata", async () => {
    const key = await deriveKey("pw", new Uint8Array(16));
    const verifier = await encryptVerifier(key);
    const meta = buildVaultAuthMetadata({ salt: new Uint8Array(16), verifier });
