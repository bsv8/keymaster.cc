...
// （Coordinator.generateKey）只接收 hex 字符串并立即交给 Catalog Hold
// 加密流程；不在 React state、MessageBus 或任何 metadata K-V 字段中保留明文。
 ...
 *   - 字节数组由 Coordinator.generateKey 立即在闭包内消费,
 *     不会出现在返回 heap 上：本函数只返回 hex 字符串，调用结束后
 *     引用链断开由 GC 回收。
 *   - 不派生地址、不写 metadata K-V repository。地址与持久化由 Coordinator 负责。
 */
