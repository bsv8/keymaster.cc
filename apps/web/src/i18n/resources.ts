  ...
  resources: {
    en: {
      ...
      "shell.setup.type.description":
        "The bucket determines where Keys and app data are stored. You can add more buckets later in Bucket management.",
      ...
      "shell.setup.parameters.s3":
        "Enter the object-store location and credentials. The browser connects directly to this bucket.",
      ...
      "shell.setup.password.description":
        "This password protects both the bucket configuration and your Keys, which are encrypted separately. It is never written to the local catalog or logs.",
      "shell.setup.password.next": "Save and continue",
      "shell.setup.keyChoice.title": "Set up your first Key",
      "shell.setup.keyChoice.description":
        "Initial setup creates or imports one Key, then opens Key management.",
      ...
      "shell.setup.newKey.description":
        "Enter a Tag Name to identify this Key. Its private key is generated securely on this device.",
      "shell.setup.newKey.tag": "Tag Name (Key label)",
      "shell.setup.newKey.placeholder": "For example: Primary Key",
      "shell.setup.newKey.submit": "Create and open Key management",
    ...
    "zh-CN": {
      ...
      "shell.setup.type.description": "桶决定 Key 和应用数据保存在哪里。以后可以在桶管理中继续添加其他桶。",
      ...
      "shell.setup.parameters.s3": "填写对象存储位置和访问凭据；浏览器会直接连接该桶。",
      "shell.setup.parameters.testNext": "测试连接并继续",
      "shell.setup.password.title": "设置密码",
      "shell.setup.password.resumeTitle": "输入密码继续初始设置",
      "shell.setup.password.description": "此密码同时保护桶配置和 Key，但底层分别加密；密码不会写入本机目录或日志。",
      "shell.setup.password.next": "保存并继续",
      "shell.setup.keyChoice.title": "设置第一把 Key",
      "shell.setup.keyChoice.description": "初始设置只创建或导入一把 Key，完成后进入 Key 管理页面。",
      ...
      "shell.setup.newKey.description": "填写用于识别这把 Key 的 Tag Name，私钥将由本机安全生成。",
      "shell.setup.newKey.tag": "Tag Name（Key 标签名称）",
      "shell.setup.newKey.placeholder": "例如：主 Key",
      "shell.setup.newKey.submit": "创建并进入 Key 管理",
