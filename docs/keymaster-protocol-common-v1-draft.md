...
### Session Window（施工单 2026-06-29 001 硬切换 + 2026-06-30 002 硬切换）
...
- `appView` mode：Keymaster launcher 拉起，URL 上加 `?boot=appView`；Session Window 在挂载时进入"等待 launcher bootstrap"状态，launcher 通过一次性同源 `__keymaster_session_window_bootstrap__.acquire(token)` 把 `AppBootstrapPayload`（**含 owner runtime bootstrap** + connectSessionId + launchToken + app 信息）移交过去。token 一次性消费，命中即从 launcher registry 删除。

两种 mode 在启动后走同一套 service / 同一套 transport / 同一套协议方法族。差异只存在于**启动阶段**。

#### Owner runtime bootstrap 替代 unlock runtime（施工单 2026-06-30 002 硬切换）
...
硬切后（2026-06-29 003 引入的中间态"session signer"已被 2026-06-30 002 撤销，最终版）：

- `appView` mode 的 Session Window 启动早期只持有 `OwnerRuntimeBootstrap`（ownerPublicKeyHex + privateKeyHex + ownerLabel + capabilities + createdAt），足够跑 `identity.*` / `intent.sign` / `cipher.*` / `p2pkh.transfer` / `feepool.*`；
...
- `AppBootstrapPayload` 字段命名为 `ownerRuntimeBootstrap: OwnerRuntimeBootstrap`（中间态 `sessionSigner: SessionSignerBootstrap` 已撤销）；
...
`UnlockRuntimeHandoff` / `vault.export/importUnlockRuntime*` / 中间态命名 `SessionSignerBootstrap` / `export_session_signer_failed` 已全部删除；现行代码不再出现 `session_signer` / `session signer` 命名。
...
- 业务方法（`identity.*` / `intent.sign` / `cipher.*` / `p2pkh.transfer` /
  `feepool.*`）**统一**走 `resolveOwnerRuntime` 这一个入口；
  **不**再按"vault / owner runtime"业务分支硬编码。
