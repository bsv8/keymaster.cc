...
8. **owner 执行面 = session 真值（施工单 2026-06-29 003 硬切换）**：
   内容 key 派生与签名 / 加解密走同一条 owner 解析路径：
   - `runtimeBinding === "vault"`：从已解锁 vault 借 owner 私钥派生；
   - `runtimeBinding === "session_signer"`：从 Session Window 内存的
     `sessionSignerRuntimes` map 拿 owner 私钥 hex 派生。
   缺 signer 时 `storage.*` 与 `cipher.*` / `intent.sign` 一致 fail-closed；
...
- 派生输入（IKM = owner 私钥 hex）按 `session.runtimeBinding` 走同一
  套 owner 解析真值（vault 路径走 `vault.withPrivateKey`；session_signer
  路径直接用 in-memory signer 私钥 hex）。两条路径**不**允许在同一
  `connectSessionId` 下混用；缺 signer 即 fail-closed。
