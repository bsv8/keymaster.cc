...
| 首次升级接管 | 首次从不认识 Web Locks 的 registry `0.4.2` 迁移必须先做部署门禁要求的冷切换；双方都支持 Web Locks 的后续升级使用立即检查，旧 Worker 存活时新 Worker 直接报错，用户刷新/关闭全部页面后重启。Keymaster `UpgradeSession` 只保护当前 Worker 内存 I/O。 | WebLoom runtime lock 单测、Coordinator Worker 单测、真实 Chromium 多 Tab 回归，以及运行锁迁移门禁自测。 | 多浏览器不是同一把锁；旧 `0.4.2` 不能被 `0.4.3` 自动发现；未知远端结果仍需领域对账，外部部署证据仍未完成。 |
