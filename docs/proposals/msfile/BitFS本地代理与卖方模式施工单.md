...
>
> 本施工单不授权修改 `go-bitfs`、`ChannelProtocol` 或 `MSFile-Proxy-Protocol` 仓库；跨仓库变更应分别立项。Keymaster 内对已安装 go-bitfs 的本地补丁仅用于买方本地签名状态的验证与重建，不修改 BitFS wire Kind 1–13。
...
5. 池内付款交易只保存双方签名、raw 与 txid，供下一轮线性递进或卖方仲裁使用；开池、最终关池、退款与仲裁交易广播结果未知时进入对账。
...
4. 开池由买方广播、最终关池由卖方广播，WOC 明确接受广播后即可推进对应状态且不等待出块；广播结果未知时按原 txid 对账。池内 Kind 7 只在双方本地按序递进，不提交 WOC，也不等待链上/内存池显示。
...
3. 池内付款先持久化 Kind 5/7 和双方本地证据，卖方认可后推进下一轮，不调用 K3 广播。最终关池交易先进 outbox，再调用 K3；广播成功后推进关池，未知结果再对账。
...
- K6（单卖家买方主流程）：页面可选已验签报价并显示文件大小、内容价估算、
  ...
  Kind 7 的签名意图、签名结果和精确报文先落入 journal；用买方已签 Kind 5/7
  验证并重建本地递进状态后提交完整文件，不等待池内交易在链上出现。开池 FundingTx 仍由既有 outbox 对账。相同 Kind 6
  重复到达时只重发已保存的 Kind 7，不再次签署付款。
- K6（付款后入库恢复）：报价验签后固定文件大小与推荐文件名；如果 Worker 在
  本地已签付款已保存、但本地 `Block → Seed → meta` 提交期间退出，下一次恢复该 Seed
  购买摘要时会重新校验暂存 Seed/Block 并幂等完成入库，不签名、不广播。普通池内付款
  只保存于双方本地，不进入卖方通用广播 outbox。
...
4. **触发资金与开池。** 首个符合自动块价的报价出现，或用户按文件强制下载，才调用 F1 聚合拆分并保护资金。Worker 按文件 Block 数和卖家并发上限分配各池 Block 预算；尚未购买 Seed 时，各候选池还预留自己的 Seed 报价，供 Seed 池故障并完成回收后接替。每池开池金额固定为 Seed 预留、该池 Block 预算和两笔池内手续费预留之和；不为每家卖家重复预留整份文件的 Block 预算。FundingTx 证据及 raw 回读后经既有 outbox 由买方广播；WOC 明确接受广播后即可发送 Kind 4，不等待出块。若广播结果未知，才按原 txid 对账，不能重新构造交易。
5. **正式传输与动态调度。** 完整 Block 单价不超过当前上限且有活动 DataChannel 的卖家进入有界并发队列；Worker 持久记录一个 Seed 的唯一付款归属和各 Block 的单池认领。Kind 6 验货暂存、Kind 7 买方签名持久化后，才把 Block 标记为该池已付款；池内递进不等待链上观察。调度器按价格优先或最近已付款速度优先分配后续 Block，并遵守每池固定 Block 预算。同一 Block 不得从多池重复购买。池预算用尽后停止领取并通过 Kind 12/13 关池回收，释放资金后才为后续卖家准备新池。
...
- 买方重新验收 Kind 13 中的完整关闭交易，再登记交易 outbox 和专款账本；WOC 明确广播成功后按原关池交易推进回收，不等待出块。账本按关闭交易原文核对实际输入（当前 go-bitfs 状态交易花费开池输出），只把回到当前买方 Key 的输出计入买方专款。池内递进状态从本地 Kind 5/7 恢复，不查询开池输出的 spender 来等待普通付款。
...
- 单卖家当前批次尚未生成 Kind 7 付款签名时，买方可以取消并复用 Kind 12/13 关池；卖方允许在已准备交付但尚未收到付款签名时处理该关池请求。取消意图先写入 journal，之后到达的 Kind 6 不会触发付款签名；只有 WOC 明确接受关池广播、且专款账本确认回收后才显示“已取消”。结果未知时仍保留资金保护。
...
- 买方验收重连报价时要求 Kind 1 与原 journal 中的 exact bytes 完全相同，再续接原 session ID。恢复阶段只重发已保存的 Kind 2、Kind 5、Kind 7 或 Kind 12；本地 Kind 5/7 验签后恢复池内序号与金额，不从链上等待池内付款，也不会重新签名或为旧池创建第二份报价/付款。
...
买方按文件 Block 数与卖家并发上限分配每池 Block 预算。Kind 6 验货和暂存、Kind 7 买方签名持久化后，可把 Block 标记为该池已付款；下一轮 Kind 6 是卖方继续接受递进的业务证据。超时或断线时保留原认领和双方已存证据，停止向该卖家签署新付款并允许其它卖家接替；已开池资金仍须关池或到期退款才解除保护。预算用尽的池停止请求并进入关池回收；确认回收后可为尚未分配的 Block 新建卖家池。
