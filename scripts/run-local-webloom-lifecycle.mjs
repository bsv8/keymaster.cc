// 在临时副本中用本地 WebLoom 0.4.2 tarball 跑严格 Chromium 生命周期验收。
// 这是源码/tarball 证据；正式 registry 0.4.2 证据由独立的
...
if (tarballVersion !== "0.4.2") {
  throw new Error(`严格本地生命周期验收要求 WebLoom 0.4.2，当前为 ${tarballVersion}`);
}
...
try {
  ...
  // 根 node_modules 链接。WebLoom 仍明确从上面的 0.4.2 tarball 安装。
  ...
  // lockfile 只在临时副本中重算；--offline 确保这次验收不偷偷改用 registry。
  ...
  if (installedManifest.version !== tarballVersion) {
    throw new Error(`临时副本实际安装的 WebLoom 版本不是 0.4.2：${String(installedManifest.version)}`);
  }

  // 这些检查在临时副本中执行，确认 TypeScript 解析到已安装的 0.4.2
  ...
  run("pnpm", ["exec", "playwright", "test", "--config=playwright.lifecycle.config.ts", "--project=chromium-lifecycle"]);
