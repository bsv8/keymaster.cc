...
test(JOURNEY_ID + "：bsv8 缺省网关设置页三功能", async ({ page, context }, testInfo) => {
  ...
  try {
    ...
    await test.step("刷新 SPI 余额与远端订阅正常", async () => {
      await refreshSpiBalanceFromPage(page, DEFAULT_SUPPLIER_ID);
      ...
      await refreshRemoteSubscriptionsFromPage(page, DEFAULT_SUPPLIER_ID);
    });

    await test.step("查询服务器账单（全新 Key 为空且翻页禁用）", async () => {
      await queryServerBillingFirstPage(page, DEFAULT_SUPPLIER_ID);
